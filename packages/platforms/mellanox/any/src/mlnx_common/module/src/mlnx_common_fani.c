...
_onlp_fani_info_get_fan(int local_id, onlp_fan_info_t* info)
{
	...
	snprintf(fullpath, sizeof(fullpath), "%s%s", PREFIX_MODULE_PATH, mlnx_platform_info->fan_fnames[(int)fru_index].status);
	...
	snprintf(fullpath, sizeof(fullpath), "%s%s", PREFIX_MODULE_PATH, mlnx_platform_info->fan_fnames[local_id].status);
	OPEN_READ_FILE(fullpath, r_data, nbytes, len);
	if (atoi(r_data) != FAN_STATUS_OK) {
	    return ONLP_STATUS_OK;
	}
    ...
    /* get fan speed */
    snprintf(fullpath, sizeof(fullpath), "%s%s", PREFIX_PATH, mlnx_platform_info->fan_fnames[local_id].r_speed_get);
    OPEN_READ_FILE(fullpath, r_data, nbytes, len);
    ...
    if (ONLP_FAN_CAPS_GET_PERCENTAGE & info->caps) {
        /* get fan min speed */
        snprintf(fullpath, sizeof(fullpath), "%s%s", PREFIX_PATH, mlnx_platform_info->fan_fnames[local_id].min);
        OPEN_READ_FILE(fullpath, r_data, nbytes, len);
        mlnx_platform_info->min_fan_speed[local_id] = atoi(r_data);

        /* get fan max speed */
        snprintf(fullpath, sizeof(fullpath), "%s%s", PREFIX_PATH, mlnx_platform_info->fan_fnames[local_id].max);
        OPEN_READ_FILE(fullpath, r_data, nbytes, len);
        mlnx_platform_info->max_fan_speed[local_id] = atoi(r_data);
...
onlp_fani_get_min_rpm(int id)
{
    ...
    if (onlp_file_read((uint8_t*)r_data, nbytes, &len, "%s%s", PREFIX_PATH, mlnx_platform_info->fan_fnames[id].min) < 0)
        return ONLP_STATUS_E_INTERNAL;
