...
#define OPEN_READ_FILE(prefix_module, path, data, nbytes, len)			\
	if (onlp_file_read((uint8_t*)data, nbytes, len, "%s/%s", prefix_module, path) < 0)	\
...
onlp_fani_get_min_rpm(int id)
{
	...
	OPEN_READ_FILE(PREFIX_PATH, fan_path[id].min, r_data, nbytes, &len);
    return atoi(r_data);
}
