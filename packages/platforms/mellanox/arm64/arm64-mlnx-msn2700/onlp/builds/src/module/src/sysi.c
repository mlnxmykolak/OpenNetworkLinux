...
int mc_get_platform_info(mlnx_platform_info_t* mlnx_platform)
{
	if (!__ONIE_PLATFORM_NAME) {
	    strncpy(mlnx_platform->onl_platform_name, "arm64-mlnx-msn2700-all", PLATFORM_NAME_MAX_LEN);
	    strncpy(mlnx_platform->onie_platform_name, "arm64-mlnx-msn2700-all", PLATFORM_NAME_MAX_LEN);
	}
	else {
...
{
    if(!strcmp(platform, "arm64-mlnx-msn2700-r0")) {
        __ONIE_PLATFORM_NAME = "arm64-mlnx_msn2700-r0";
        return ONLP_STATUS_OK;
    }
    if(!strcmp(platform, "arm64-mlnx-msn2700b-r0")) {
        __ONIE_PLATFORM_NAME = "arm64-mlnx_msn2700b-r0";
        return 0;
    }
    if(!strcmp(platform, "arm64-mlnx-msn2700-all")) {	/* Is it required? */
    	__ONIE_PLATFORM_NAME = "arm64-mlnx-msn2700-all";
    	return 0;
    }
